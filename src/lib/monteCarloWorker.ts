...
interface SimRequest {
  ...
  regionNames: string[];
  teamSeeds: Record<string, number>; // region-seed -> seed number
...
interface SimResult {
  type: "progress" | "done";
  progress?: number;
  results?: { key: string; avgScore: number; avgPlace: number; winPct: number }[];
}
...
function feeders(
  ...
  if (region === "ff") {
    if (round === 4 && idx === 0)
      return [`${regionNames[0]}-3-0`, `${regionNames[2]}-3-0`];
    if (round === 4 && idx === 1)
      return [`${regionNames[1]}-3-0`, `${regionNames[3]}-3-0`];
    if (round === 5) return ["ff-4-0", "ff-4-1"];
    return null;
  }
  if (round === 0) return null; // R64 teams come from seeding
