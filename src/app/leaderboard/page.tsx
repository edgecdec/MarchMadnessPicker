                ...
                {leaderboard.map((entry, i) => (
                  <TableRow key={`${entry.username}-${entry.bracket_name || i}`}>
                    ...
                    <TableCell
                      ...
                      sx={{ maxWidth: 140, whiteSpace: "nowrap", cursor: locked && entry.ffPicks ? "default" : undefined }}
                    >
                      <Tooltip title={`${entry.username}${entry.bracket_name ? ` — ${entry.bracket_name}` : ""}`}>
                        <Box sx={{ overflow: "hidden", textOverflow: "ellipsis", maxWidth: 130 }}>
                          <Link href={`/bracket/${entry.username}`} underline="hover" sx={{ fontSize: "0.85rem" }}>{entry.username.length > 12 ? entry.username.slice(0, 11) + "…" : entry.username}</Link>
                          {entry.bracket_name && <Box component="span" sx={{ color: "text.secondary", fontSize: "0.75rem" }}>{` · ${entry.bracket_name.length > 10 ? entry.bracket_name.slice(0, 9) + "…" : entry.bracket_name}`}</Box>}
                        </Box>
                      </Tooltip>
