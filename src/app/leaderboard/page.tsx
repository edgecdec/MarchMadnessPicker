            ...
            <Table size="small" stickyHeader sx={{ tableLayout: "fixed" }}>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={{ position: "sticky", left: 28, top: 0, zIndex: 4, bgcolor: "background.paper", width: 128, minWidth: 128, px: 0.5, fontSize: "0.8rem" }}>
                    <TableSortLabel active={orderBy === "player"} direction={orderBy === "player" ? order : "asc"} onClick={() => handleSort("player")}>Player</TableSortLabel>
                  </TableCell>
                  <TableCell align="right" sx={{ position: "sticky", left: 156, top: 0, zIndex: 4, bgcolor: "background.paper", borderLeft: 1, borderColor: "divider", width: 36, minWidth: 36, maxWidth: 36, px: 0.5, fontSize: "0.8rem" }}>
                ...
                {sortedIndices.map((idx) => { const entry = leaderboard[idx]; const i = idx; const isOwn = entry.username === user?.username; return (
                  <TableRow key={`${entry.username}-${entry.bracket_name || idx}`} sx={isOwn ? { bgcolor: "action.selected", borderLeft: 3, borderColor: "primary.main" } : undefined}>
                    ...
                    <TableCell
                      ...
                      sx={{ position: "sticky", left: 28, zIndex: 1, bgcolor: isOwn ? "action.selected" : "background.paper", width: 128, minWidth: 128, whiteSpace: "nowrap", cursor: locked && entry.ffPicks ? "default" : undefined, py: 0.5, px: 0.5 }}
                    >
                      <Tooltip title={`${entry.username}${entry.bracket_name ? ` — ${entry.bracket_name}` : ""}`}>
                        <Box sx={{ display: "flex", alignItems: "center", width: "100%" }}>
                          <Box sx={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", minWidth: 0, flex: 1 }}>
                            <Link href={`/bracket/${entry.username}`} underline="hover" sx={{ fontSize: "0.8rem" }}>{entry.username}</Link>
                            {entry.bracket_name && <Box component="span" sx={{ color: "text.secondary", fontSize: "0.8rem" }}>{" - "}{entry.bracket_name}</Box>}
                          </Box>
                          <Box component="span" sx={{ flexShrink: 0, ml: 0.5, fontSize: "0.75rem", whiteSpace: "nowrap" }}>
                            {locked && entry.busted && <Tooltip title={`Championship pick eliminated: ${entry.championPick}`}><span>💀</span></Tooltip>}{locked && (() => { const key = `${entry.username}|${entry.bracket_name || ""}`; const maxP = trueMax[key] != null ? trueMax[key] : entry.score + (entry.maxRemaining ?? 0); const top = leaderboard.length > 0 ? leaderboard[0].score : 0; return maxP < top ? <Tooltip title="Eliminated from contention — cannot catch the leader"><span>🚫</span></Tooltip> : null; })()}{locked && (() => { const s = computeHotStreak(entry.picks, results || {}); return s >= 5 ? <Tooltip title={`${s} correct picks in a row`}><span>{s}🔥</span></Tooltip> : null; })()}{locked && regions && (() => { if (!entry.picks) return null; const elim = getEliminatedTeams(results || {}, regions); const e8Keys = regions.map(r => `${r.name}-3-0`); const ffPicks = e8Keys.map(k => entry.picks![k]).filter(Boolean); if (ffPicks.length < 4) return null; const allElim = ffPicks.every(p => elim.has(p)); const gotAny = e8Keys.some(k => results?.[k] && entry.picks![k] === results![k]); return allElim && !gotAny ? <Tooltip title="Entire Final Four eliminated"><span>🤡</span></Tooltip> : null; })()}{(() => { const key = `${entry.username}|${entry.bracket_name || ""}`; const u = uniquePicks[key]; return u?.length ? <Tooltip title={<>{u.map((p, i) => <div key={i}>{p}</div>)}</>}><span>{u.length}😱</span></Tooltip> : null; })()}
                          </Box>
                        </Box>
                      </Tooltip>
                    </TableCell>
                    <TableCell align="right" sx={{ position: "sticky", left: 156, zIndex: 1, bgcolor: isOwn ? "action.selected" : "background.paper", borderLeft: 1, borderColor: "divider", fontWeight: "bold", width: 36, minWidth: 36, maxWidth: 36, px: 0.5, fontSize: "0.85rem", cursor: "pointer", textDecoration: "underline", "&:hover": { color: "primary.main" } }} onClick={() => openBreakdown(entry)}>{entry.score}</TableCell>
