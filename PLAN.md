...
- [x] Add a medium/tablet bracket layout for screens that can fit half but not the full bracket: Currently we have mobile (round tabs) and full desktop (all 4 regions + FF). Add a third layout for medium screens (~768-1200px). Design: stack all 4 regions vertically on the left side, each showing R64 on the left flowing to E8 on the right (16→1 seed direction). Final Four and Championship shown in a flatter horizontal layout at the top and/or bottom. This way users can see one full region at a time without horizontal scrolling. Research how this could look — study ESPN's tablet bracket view for inspiration. Write a brief spec to specs/medium-bracket.md before implementing. Detect screen width and switch between mobile (<768), medium (768-1200), and desktop (>1200) layouts.

- [x] Refactor simulator page to reuse the same bracket layouts (mobile/medium/desktop) as the main bracket page: The simulator currently has its own bracket rendering. It should use the same responsive layout components so all three views (mobile tabs, medium stacked, desktop full) work on the simulator too. Extract the layout switching logic into a shared component or hook.

- [x] Research and implement a maintenance page during deployments: During deploys the site can be briefly broken (missing chunks, 502s). Research how to show a simple "This website is under maintenance, check back shortly" page during deployments. Options to consider: (1) nginx serving a static maintenance.html when the app is down (detect pm2 stop or 502 upstream). (2) A deploy_webhook.sh step that swaps nginx to serve a static page before stopping pm2, then swaps back after restart. (3) A client-side approach with a service worker that shows a cached maintenance page on network errors. Pick the simplest approach that works with our nginx + pm2 setup. Document findings in specs/maintenance-page.md before implementing.
