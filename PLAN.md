...
- [x] Auto-fill incomplete brackets at lock time using "Smart" autofill: When the tournament lock time hits, any bracket that isn't fully filled out (less than 63 picks) should automatically get its remaining empty slots filled using the Smart autofill (historical seed win rates). This ensures every bracket is complete for scoring. Run this as part of the lock process or as a check when lock time passes.
