...
- [2026-03-15 03:13] **Viewing own bracket at /bracket/[username] says "picks are locked"**: When a user visits /bracket/edgecdec (their own username), it shows the read-only view with "🔒 Picks are locked" even though picks aren't actually locked. This is confusing. Fix: if the logged-in user is viewing their own bracket, either redirect to /bracket (the editable page) or change the message to "Viewing your bracket (edit at /bracket)" instead of showing the lock message.

- [2026-03-15 03:30] **No unsaved changes warning when leaving bracket page**: If a user makes picks on the bracket page but hasn't saved, navigating away (clicking a nav link, closing tab, or hitting back) loses their picks silently. Fix: track whether picks have changed since last save (compare current picks to initialPicks). If there are unsaved changes, use `beforeunload` event to warn on tab close, and intercept Next.js route changes to show a confirmation dialog like "You have unsaved picks. Leave without saving?"
